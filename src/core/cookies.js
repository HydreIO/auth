...
export const accessCookie = ({ COOKIE_DOMAIN, ACCESS_COOKIE_NAME }) => rememberMe => accessToken => {
	const payload = { httpOnly: true, secure: false, sameSite: 'strict', ...prodOpt(COOKIE_DOMAIN) }
...
export const refreshCookie = ({ COOKIE_DOMAIN, REFRESH_COOKIE_NAME }) => refreshToken => {
	const payload = { httpOnly: true, secure: false, maxAge: aYear, sameSite: 'strict', ...prodOpt(COOKIE_DOMAIN) }
...
export const expiredAccessCookie = ({ COOKIE_DOMAIN, ACCESS_COOKIE_NAME }) => {
	const payload = { httpOnly: true, secure: false, sameSite: 'strict', expires: new Date(0), ...prodOpt(COOKIE_DOMAIN) }
