...
import { SessionError, CookiesError } from '../graphql/errors'
...
export const buildContext = env => collection => sso => event => {
	...
	const eventOps = new EventOps(event, env)
	return {
		@cache
		async getUser(canAccessTokenBeExpired) {
			const token = eventOps.parseAccessToken() || throw new CookiesError()
			const user = userOps.fromToken(token) |> userOps.loadSession(ip, findUserAgent(event))
