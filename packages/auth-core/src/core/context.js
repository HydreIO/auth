...
export const buildContext = ({ env, event, crud, socketOps }) => {
  const eventOps = EventOps({
    env,
    headers: event.headers,
    addCookie: event.addCookie,
  })
  let cachedUser
  return {
    /**
     ...
     */
    async getUser({
      canAccessTokenBeExpired = false,
      checkForCurrentSessionChanges = true,
    } = {}) {
      ...
      debug('.......retrieving user %O', {
        canAccessTokenBeExpired,
        checkForCurrentSessionChanges,
      })
      const token = eventOps.parseAccessToken()
      if (!token) throw new CookiesError()
      const user = userOps.loadSession(
        env.IP,
        eventOps.parseUserAgent(event)
      )(userOps.fromToken(env)(token))
      ...
      const session = userOps.getSessionByHash(
      ...
      if (!canAccessTokenBeExpired && user[Symbol.transient].sessionExpired)
        throw new SessionError()
    ...
    socketOps,
    sso: {
      verifyGoogleIdToken: verifyGoogleIdToken(googleClient)(env.GOOGLE_ID),
    },
  }
}
