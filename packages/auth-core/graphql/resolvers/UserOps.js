...
import { TooManyRequestError, BadEmailFormatError, UnknowCodeError, BadPwdFormatError, InvalidResetCodeError, InvalidVerificationCodeError, InvalidRefreshTokenError } from '../errors'
...
export const inviteUser = async (_, { email }, { getUser, crud: { fetch, exist, push }, env: { PRV_KEY, LABEL, EMAIL_REGEX, INVITE_USER_DELAY } }) => {
	...
	const invited = { uuid: uuidv4(), email, hash: undefined, sessions: [], resetCode }
	debug('......presaving invited user')
	await push(invited.uuid, invited)
	...
	debug('......signing jwt')
	return signJwt(PRV_KEY)(jwtOptions)({ uuid: invited.uuid, email })
}
...
export const sendCode = async (_, { code, email }, { env: { LABEL, RESET_PASS_DELAY, CONFIRM_ACCOUNT_DELAY, EMAIL_REGEX }, crud: { push, fetch } }) => {
	...
	if (user) {
		...
		switch (code) {
			case 'RESET_PWD':
				// allowing this query only once every X ms
				if (user.lastResetEmailSent + RESET_PASS_DELAY > Date.now()) throw new TooManyRequestError()
			...
			case 'CONFIRM_EMAIL':
				...
				if (user.lastVerifEmailSent + CONFIRM_ACCOUNT_DELAY > Date.now()) throw new TooManyRequestError()
				// if the code already exist we retrieve it, if not we create a new one
				user.verificationCode ||= shaCode(email)
				user.lastVerifEmailSent = Date.now()
				debug('......emailing confirm code')
				events.emit(EVENTS.CONFIRM_EMAIL, { to: email, code: user.verificationCode })
				debug('......updating user')
				await push(user.uuid, user)
				break
