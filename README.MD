...
This query will return an error if the env variable `DEVELOPMENT` is not set to `true`
...
returns a signed JWT with the `invitedId` (userid) and `email` of the newly created user
